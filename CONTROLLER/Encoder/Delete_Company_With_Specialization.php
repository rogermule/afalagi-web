	...
	if(TRUE == check_login_status()){
		...
		if(($user_type == User_Type::ENCODER) or ($user_type == User_Type::NORMAL_ENCODER)){
			...
			if(empty($errors)){
				...
				if(!$with_building){
					$deleted = $encoder_con->Delete_Company_With_Out_Building_With_Specialization($Company_ID,
						$About_Company_ID,
						$Payment_Status_ID,
						$Company_Service_ID,
					...
					$deleted =$encoder_con->Delete_Company_With_Building_With_Specialization($Company_ID,
						$About_Company_ID,
						$Payment_Status_ID,
						$Company_Service_ID,
						$Company_Ownership_ID,
