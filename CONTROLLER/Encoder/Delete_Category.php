...
function encoder_redirect_success(){
	...
	$dir = "VIEW/html/Encoder/Add_Category/Category_List.php?success_delete=1";
	$url = BASE_URL.$dir;
	header("Location:$url");//redirect the encoder to the Categorys add place
	exit();
...
function encoder_place_redirect($type_of_error,$Category_ID){
	...
	if($type_of_error == Error_Type::FORM){
		...
		$error_type = "The Category is Used by Company or Building, You can not delete it. If You want it to be deleted, First Deleted things that reference this Category, like Building or company.";
	}
	else if($type_of_error == Error_Type::SAME_USER_NAME){
		$error_type = "Error Same Category Name. Same Category name can not be added";
	}
	$dir = "VIEW/html/Encoder/Add_Category/Delete_Category.php?error=$error_type&Category_ID=$Category_ID";
	$url = BASE_URL.$dir;
	header("Location:$url");
	exit();
	...
	if(TRUE == check_login_status()){
 		...
 		if($user_type == User_Type::ENCODER){
			...
			if(empty($_POST['Category_ID'])){
 			...
 			if(empty($errors)){
 				$deleted = $encoder_con->Delete_Category($Category_ID);
 				if($deleted){
