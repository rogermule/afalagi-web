...
function encoder_redirect_success(Company $company){
	...
	$dir = "VIEW/html/Encoder/Add_Company/Add_Company.php?success=1&Company_Name=$Company_Name&Company_Name_Amharic=$Company_Name_Amharic";
	$url = BASE_URL.$dir;
	...
	if(TRUE == check_login_status()){
		...
		if(($user_type == User_Type::ENCODER) or ($user_type == User_Type::NORMAL_ENCODER)){
			...
			if(isset($_POST['Specialization'])){
				...
				//get the array and put them in array
			...
			if(empty($errors)){
				...
				if($with_building){
					...
					$added = $encoder_con->Add_Company_With_Building_With_Specialization($Company_Name,$Company_Name_Amharic,$Category_ID,$Company_Type_ID,$Branch,$Branch_Amharic,$Working_Hours,$Working_Hours_Amharic,$Product_Description_And_Service,$Product_Description_And_Service_Amharic,$Registration_Expiration_Date,$Registration_Type,$Building_ID,$Building_Floor,$Contact_C,$specialization);
